...
def add_set_nm_id(detail):
    result = {11111111, 22222222, 33333333, 44444444}
    for line in detail:
        nm_id = line.nm_id
        if nm_id != 0:
            result.add(nm_id)
...
def aggregation_by_item(detail, set_nm_id):
    result = []
    other_nm_id = {'Семена': 11111111, 'Блузки': 22222222, 'Рубашки': 33333333, 'Брюки': 44444444}
    for nm_id in set_nm_id:
        ...
        if sa_name != '':
            result.append(
                {'nm_id': nm_id, 'name': sa_name, 'count': round(count, 2), 'sales': round(sales, 2),
                 'commission': round(commission, 2), 'logistic': round(logistic, 2)})
    return result


# def control_method(realizationreport, mapping_data):
#     sales = 0
#     comiss = 0
#     logistic = 0
...
#     if realizationreport.sales != sales:
#         raise ValueError('sales are not equal')
#     elif (realizationreport.sales - realizationreport.transfer_for_goods) != comiss:
#         raise ValueError('comission are not equal')
#     elif realizationreport.logistic != logistic:
#         raise ValueError('logistic are not equal')
#     else:
#         return True
